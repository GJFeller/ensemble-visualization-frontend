    ...
    for (var item in data) {
      ...
      for (const [key, points] of Object.entries(data[item])) {
        ...
        points.forEach((point) => {
          ...
          groupsAreaTemp[point[0]].yMin = Math.min(
            groupsAreaTemp[point[0]].yMin,
            point[1],
          );
          groupsAreaTemp[point[0]].yMax = Math.max(
            groupsAreaTemp[point[0]].yMax,
            point[1],
          );
        });
      }
      ...
      for (const [simulation, points] of Object.entries(data[ensemble])) {
        svg
          ...
          .on("touchmouse mousemove", function (event) {
            ...
            const bisectionIndex = yearBisector(
              data[ensemble][simulation],
              year,
            );
  ...
  static #drawScatterPlot(chartId, chartSettings) {
    ...
    if (chartSettings.drSettings.showConvexHull) {
      ...
      for (const ensemble of groups) {
        if (convertedData[ensemble].length < 3) canConvexHull = false;
      }
      if (canConvexHull) {
